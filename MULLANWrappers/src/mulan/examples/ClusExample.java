...
public class ClusExample {
	...
	public static void main(String[] args) throws Exception {
		...
		String updated_settings_file_name = settings_file_name+"some_random_suffix";
		Scanner in = new Scanner(new FileInputStream(settings_file_name+".s"));
		BufferedWriter out = new BufferedWriter(new FileWriter(updated_settings_file_name+".s"));
		...
		//we create a new settings file and update the file_names (because the original ones are invalid, we first have to convert them)
		while ( in.hasNext() ) {
			String line = in.nextLine();
			if ( line.startsWith("File") ) {
				String file_name = line.substring(line.indexOf('=')+2);
				try { Integer.parseInt(file_name); }
				catch ( Exception e ) { 
					train_file_name = file_name;
					train_file_name = train_file_name.substring(0,train_file_name.indexOf('.'));
					updated_train_file_name = train_file_name+"some_random_suffix"+".arff";
			...
			if ( line.startsWith("TestSet") ) {
				...
				try { Integer.parseInt(file_name); }
					...
					test_file_name = test_file_name.substring(0,test_file_name.indexOf('.'));
					updated_test_file_name = test_file_name+"some_random_suffix"+".arff";
					line = line.substring(0,line.indexOf('=')+2);
			...
			if ( line.startsWith("PruneSet") ) {
				...
				try { Integer.parseInt(file_name); }
					...
					prune_file_name = prune_file_name.substring(0,prune_file_name.indexOf('.'));
					updated_prune_file_name = prune_file_name+"some_random_suffix"+".arff";
		...
		if ( load_hier_xml_pos == -1 ) {
			...
			if ( num_class_labels_pos != -1 ) {
				try {
					num_class_labels = Integer.parseInt(getArgument(args,num_class_labels_pos));
		...
		//we set the input_file_name, and then tell the converter to save our converted file and our hierarchy
		String hierarchy_file_name = save_hier_xml_name;
		converter_args.add(0,train_file_name);
		converter_args.add("-C");
		converter_args.add(updated_train_file_name.substring(0,updated_train_file_name.indexOf('.')));
		converter_args.add("-S");
		converter_args.add(save_hier_xml_name);
		ConverterHOMERMULANtoClus.main(converter_args.toArray(new String[converter_args.size()]));
		//if we have a training set we have to convert it as well, instead of building the hierarchy twice, we use the hierarchy xml
		if ( use_test ) {
			...
			converter_args.add(save_hier_xml_name);
			converter_args.add("-C");
			converter_args.add(updated_test_file_name.substring(0,updated_test_file_name.indexOf('.')));
			ConverterHOMERMULANtoClus.main(converter_args.toArray(new String[converter_args.size()]));
		...
		if ( use_prune ) {
			...
			converter_args.add(save_hier_xml_name);
			converter_args.add("-C");
			converter_args.add(updated_prune_file_name.substring(0,updated_prune_file_name.indexOf('.')));
			ConverterHOMERMULANtoClus.main(converter_args.toArray(new String[converter_args.size()]));
			
		}
		//we get all the clus arguments and set the settings file name, then we run clus
		int clus_args_pos = Utils.getOptionPos("-clus", args);
		String clus_args[] = removeEmptyStrings(getArguments(args, clus_args_pos, 1100));
		clus_args = Arrays.copyOf(clus_args, clus_args.length+1);
		for ( int i = 1 ; i < clus_args.length ; ++i ) clus_args[i] = clus_args[i-1];
